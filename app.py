...
def save_cost_log(log: list):
    """cost_log.json にログを書き込む。"""
    try:
        COST_LOG_FILE.write_text(json.dumps(log, ensure_ascii=False, indent=2), encoding="utf-8")
    except Exception:
        pass


def add_cost_log(label: str, cost: float, mode: str):
    """コストログにエントリを追加し、ファイルに永続化する。"""
    if "cost_log" not in st.session_state:
        st.session_state.cost_log = load_cost_log()
    entry = {
    ...
    save_cost_log(st.session_state.cost_log)
...
def build_system_prompt(samples):
    instruction_block = {
        ...
        "text": (
            ...
            "## 歴史・時事・未来の活用\n"
            "### 過去の歴史的事例\n"
            "- リーマンショック・コロナショック・バブル崩壊・東日本大震災・ITバブル崩壊など、ジャンルに関連する歴史的事例は積極的に活用してよい\n"
            "- 歴史的事例に**具体的な数字は使わない**。「数ヶ月で大きく動いた」「数十円規模の暴落」のような定性的表現にとどめること\n"
            "- 歴史的事例には必ず**当時の社会情景・人々の感情・日本の空気感**を添えること\n"
            "  （例：「あの頃、街中では会社が次々と倒産し」「テレビでは毎日〜というニュースが流れていた」「誰もが明日の仕事を不安に思っていた時期でした」）\n\n"
            "### 未来への警告・社会課題\n"
            "以下の社会課題・未来リスクの中からジャンルに関連するものを選んで台本に盛り込んでよい。\n"
            "具体的な数字は使わず、定性的な表現・人々の感情・生活への影響で描くこと。\n"
            "- **老後資金不足**：老後2000万円問題、年金だけでは生活できない時代、長生きリスク、人生100年時代\n"
            "- **AI・自動化による雇用喪失**：ホワイトカラーの仕事がAIに置き換わる、今の職種が10年後になくなる、スキルの陳腐化\n"
            "- **終身雇用・会社依存の崩壊**：大企業でも安泰ではない、リストラ・早期退職、副業解禁の背景にある会社の本音\n"
            "- **物価上昇・インフレ・円安**：給料は上がらないのに生活費だけ上がる、実質賃金の低下、円の購買力低下\n"
            "- **社会保険料・税負担の増大**：手取りが年々減っていく、給料明細を見るたびに引かれる額が増えている\n"
            "- **少子高齢化・労働力不足**：支える人が減り一人当たりの負担が増える、社会保障の持続可能性への不安\n"
            "- **格差社会の拡大**：資産を持つ人と持たない人の差が広がる、知識・スキル格差が収入格差に直結する時代\n"
            "- **デジタル化・DX格差**：対応できる人とできない人で収入が大きく変わる、乗り遅れると取り戻せない\n\n"
            "### 時間軸のつなぎ方\n"
            "- 最新のトレンド情報が提供された場合：**過去の事例 → 最近のニュース → ターゲットの悩み**の順で自然につなげること\n"
            "- 未来リスクを使う場合：**現在の不安 → このまま何もしないと〜 → だから今動く必要がある**の流れで緊急性を高めること\n\n"
            "## 生成の進め方\n"
...
「---構成案ここまで---」と書いた後、以下のフォーマットで「Claude向け台本生成プロンプト」を出力してください。

---Claude向け台本生成プロンプト（コピペ用）---

以下の骨子をもとに、{info.get("seller_name", "販売者")}さんの{episode_count}話構成の販売台本を生成してください。

【販売者】{info.get("seller_name", "販売者")}（一人称：{info.get("seller_first_person", "私")}）
【商品名】{info.get("name", "")}
【ターゲット】{info.get("target", "")}
【構成】{episode_structure}

【絶対ルール】
- 書き言葉（〜である・〜のである・〜といえる・〜とされている）は絶対禁止。すべて話し言葉で書く
- 1文は短く。読点で区切り、一文に複数の意味を詰め込まない
- 販売者の一人称は「{info.get("seller_first_person", "私")}」。口癖・人間味・キャラクターを全話一貫して描く
- 冒頭15秒以内に「この動画を最後まで見る理由」を明示するフックを入れる
- 視聴者との対話感を出す（「〜ですよね」「わかります」「実は私も〜」）
- 各話の【感情目標】に沿って感情の山谷を作ること。一本調子にしない
...
""")
    ...
    if "cost_log" not in st.session_state:
        st.session_state.cost_log = load_cost_log()
    _cost_log = st.session_state.cost_log

    _total_usd = sum(e["cost"] for e in _cost_log)
    col_cost, col_reset = st.columns([3, 1])
    with col_cost:
        st.metric("累計合計", f"${_total_usd:.4f}", f"約{_total_usd * _JPY_RATE:.0f}円")
    with col_reset:
        st.write("")
    ...
    if _cost_log:
        ...
        for _e in _cost_log:
            _by_date[_e.get("date", "不明")].append(_e)
        for _date in sorted(_by_date.keys(), reverse=True):
            _day_entries = _by_date[_date]
            _day_total = sum(e["cost"] for e in _day_entries)
            with st.expander(f"{_date}　{fmt_cost(_day_total)}", expanded=(_date == datetime.now().strftime("%Y-%m-%d"))):
                for _e in reversed(_day_entries):
                    st.markdown(
                        f"**{_e['time']}** {_e['label']}  \n"
                        f"{fmt_cost(_e['cost'])}　`{_e['mode']}`"
                    )
    else:
